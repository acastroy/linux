...
int pci_i2c_multi_read(struct tw5864_dev *dev, u8 devid, u8 devfn, u8 *buf, u32 count)
{
    int i = 0;
    u32 val = 0;
...
int pci_i2c_multi_write(struct tw5864_dev *dev, u8 devid, u8 devfn, u8 *buf, u32 count)
{
    int i = 0;
    u32 val = 0;
    int timeout = TW5864_IIC_TIMEOUT;
...
void pci_init_tw2864(struct tw5864_dev *dev, u8 iic)
{
    ...
    for(ch = 0 ; ch < 4 ; ch++)
    {
        pci_i2c_multi_write(dev , iic , ch * 0x10 , tbl_pal_tw2864_common , 16);
    }
#ifdef PLATFORM_FPGA_155
...
#ifdef PLATFORM_FPGA_155
    pci_i2c_wscatter(dev , iic , tbl_tw2865_other1 , sizeof(tbl_tw2865_other1)>>1);
#else
    pci_i2c_wscatter(dev , iic , tbl_tw2865_other1 , sizeof(tbl_tw2865_other1)>>1);
#endif
...
static int tw28xx_clkp_delay(struct tw5864_dev *dev, u8 devid, u32 base_ch, u32 limit)
{
    if(dev && (base_ch < ISIL_PHY_VD_CHAN_NUMBER) && (limit <= (ISIL_PHY_VD_CHAN_NUMBER>>2))) {
        int delay;
        u8 flags;
