				    ...
				    u8 *buf)
{
	...
	dev_info(&dev->pci->dev, "internal i2c read: devid %hhx, addr %hhx\n",
		 devid, addr);
	...
	dev_info(&dev->pci->dev, "internal i2c read result: devid %hhx, addr %hhx, retries left: %d, val %hhx\n",
		 devid, addr, retries, val);
			     ...
			     int size, union i2c_smbus_data *data)
{
	...
	if (read_write == I2C_SMBUS_READ)
		ret = tw5864_i2c_read_internal(dev, ctx->devid, command, &data->byte);
	else
		ret = tw5864_i2c_write_internal(dev, ctx->devid, command, data->byte);
...
int tw5864_i2c_read(struct tw5864_dev *dev, u8 i2c_index, u8 offset, u8 *data)
{
	...
	dev_info(&dev->pci->dev, "internal i2c read: i2c_index %hhu, offset %hhx\n",
		 i2c_index, offset);
...
int tw5864_i2c_write(struct tw5864_dev *dev, u8 i2c_index, u8 offset, u8 data)
{
	...
	WARN_ON(i2c_index > 3);
	WARN_ON(!client);
	WARN_ON(!client->adapter);
	WARN_ON(client->adapter != &dev->i2c[i2c_index].adap);
	WARN_ON(client->adapter->algo != &tw5864_i2c_algo);
	WARN_ON(!client->adapter->algo);
...
int tw5864_i2c_init(struct tw5864_dev *dev)
{
	...
	for (i = 0; i < 4; i++) {
		ctx = &dev->i2c[i];
		adap = &ctx->adap;
		WARN_ON(adap != &dev->i2c[i].adap);
		WARN_ON(adap != &(dev->i2c[i].adap));
		client = &ctx->client;

		ctx->dev = dev;
		snprintf(adap->name, sizeof(adap->name),
			 "tw5864 i2c adapter #%d", i);
		...
		snprintf(client->name, sizeof(adap->name),
			 "i2cclnt#%d", i);

		WARN_ON(client->adapter != &dev->i2c[i].adap);
		WARN_ON(client->adapter != adap);
		WARN_ON(adap != &ctx->adap);
		WARN_ON(&ctx->adap != &dev->i2c[i].adap);
		WARN_ON(client->adapter->algo != &tw5864_i2c_algo);
		WARN_ON(!client->adapter->algo);
		WARN_ON(client->adapter->algo_data != &dev->i2c[i]);
		WARN_ON(client->adapter->dev.parent != &dev->pci->dev);

		dev_info(&dev->pci->dev, "client's adapter: '%s'\n", client->adapter->name);
	}
